...
const TESS_WORKER_PATH = "https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/worker.min.js";
const TESS_CORE_PATH   = "https://cdn.jsdelivr.net/npm/tesseract.js-core@5/tesseract-core.wasm.js";
const TESS_LANG_PATH   = "https://tessdata.projectnaptha.com/4.0.0";

// Smart blank thresholds (conservative)
const INK_VERY_LOW = 0.005; // 0.5%
const INK_LOW      = 0.012; // 1.2%
...
function updateSlideByIndex(idx) {
  ...
  if (badNote) badNote.textContent = pages[idx].reason || "Обнаружена плохо читаемая страница. Рекомендуем заменить для точного результата.";
  if (warnNote) warnNote.textContent = pages[idx].reason || "Текст распознан частично. Для точности лучше переснять страницу.";
}

// ---------------- BAD/WARN list under panel ----------------
...
async function createThumbAndOcr(file) {
  ...
  try {
    ...
    const fastOcrBlob = await renderJpegBlob(img, 900, 0.78);   // micro-OCR (fast)
    const ocrBlob     = await renderJpegBlob(img, 1600, 0.82);  // main OCR
  ...
  if (!ocrWorker) {
    ocrWorker = await Tesseract.createWorker(lang, 1, {
      logger: (m) => {
        ...
        pLine1.textContent = "Анализ идёт";
        pLine2.textContent = (pct !== null) ? `${m.status}: ${pct}%` : `${m.status}`;
      },
...
// ---------------- OCR one page (smart blank precheck) ----------------
async function ocrOnePage(pageObj, idx, total) {
  ...
  let inkRatio = 0;
  try {
    inkRatio = await estimateInkRatio(pageObj.fastOcrBlob || pageObj.ocrBlob);
  } catch(e) {
    inkRatio = 0.02;
  }
  pageObj.metrics = Object.assign({}, pageObj.metrics, { inkRatio });

  // 2) VERY blank -> micro OCR once
  if (inkRatio <= INK_VERY_LOW) {
    const worker = await getOcrWorker(OCR_LANG_FAST);
  ...
  const r1 = await worker.recognize((inkRatio <= INK_LOW) ? (pageObj.fastOcrBlob || pageObj.ocrBlob) : pageObj.ocrBlob);

  const rawText1 = (r1.data && r1.data.text) ? r1.data.text : "";
  let cleaned = cleanOcrText(rawText1);
